...
export class ProofPoeComponent implements OnInit {
  ...
  constructor(
    ...
    private poeDataService: PoeDataService,
  ...
  ngOnInit() {
    ...
    this.proofBotParams.params.txn = this.routerParams.params.txn;
  }
  ...
  getProofData(tdpId: string): void {
    this.poeDataService.getHashValues(tdpId).subscribe(
      data => {
        ...
        if (this.transactionData.TxnType != "tdp") {
          ...
          this.error = {
            ...
            errorMessage:
              "Proof type is not associated with the current ID. Check if the entered ID is correct and try again.",
        ...
        if (
          this.transactionData.DbHash.toUpperCase() ===
          this.transactionData.BcHash.toUpperCase()
        ) {
          this.hashesMatch = true;
        }

        //  console.log("Hashes: ", this.hashesMatch);
        this.poeDataService.getTransactionData(tdpId).subscribe(
          data => {
            //   console.log("Base64: ", data);
            this.tracifiedCoreB64Data = data.data;
            this.tracifiedCoreData = JSON.parse(
              decode(this.tracifiedCoreB64Data)
            );
